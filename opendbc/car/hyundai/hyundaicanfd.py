...
def create_steering_messages(packer, CP, CAN, enabled, lat_active, apply_torque, frame, torque_fault, left_lane, right_lane, left_lane_depart, right_lane_depart, lkas_icon, vEgo):
  values = {
    "LKA_OptUsmSta": 2,
    ...
    "LKA_RcgSta": 0,
    "Damping_Gain": 50 if vEgo < 29.0576 else 85,  # can potentially tuned for better perf [3, 200]
  }
...
def create_lfahda_cluster(packer, CAN, enabled, lfa_icon, can_canfd_blended):
  hda_icon = "HDA_Icon_State" if can_canfd_blended else "HDA_ICON"
  lfa_icon = "LFA_Icon_State" if can_canfd_blended else "LFA_ICON"

  values = {
    hda_icon: 1 if enabled and not can_canfd_blended else 0,
