...
class CarController(CarControllerBase, EsccCarController, LongitudinalController, MadsCarController):
  ...
  def update(self, CC, CC_SP, CS, now_nanos):
    ...
    if self.frame % 100 == 0 and not ((self.CP.flags & (HyundaiFlags.CANFD_CAMERA_SCC | HyundaiFlags.CAN_CANFD_BLENDED)) or self.ESCC.enabled) and \
  ...
  def create_can_msgs(self, apply_steer_req, apply_torque, torque_fault, set_speed_in_units, accel, stopping, hud_control, actuators, CS, CC, can_canfd_blended):
    ...
    if self.CP.openpilotLongitudinalControl and not can_canfd_blended:
