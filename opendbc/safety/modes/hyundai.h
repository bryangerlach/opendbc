...
#define HYUNDAI_COMMON_RX_CHECKS(legacy)                                                                                                                                               \
  ...
  {.msg = {{0x386, 0, 8, 50U, .ignore_checksum = (legacy), .ignore_counter = (legacy), .max_counter = (legacy) ? 0U : 15U, .ignore_quality_flag = true}, { 0 }, { 0 }}}, \
  {.msg = {{0x394, 0, 8, 50U, .ignore_checksum = (legacy), .ignore_counter = (legacy), .max_counter = (legacy) ? 0U : 7U, .ignore_quality_flag = true}, { 0 }, { 0 }}},  \
...
static uint32_t hyundai_compute_checksum(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x386U) {
    ...
    if (hyundai_can_canfd_blended && (msg->addr == 0x421)) {
...
static bool hyundai_tx_hook(const CANPacket_t *msg) {
  ...
  const TorqueSteeringLimits HYUNDAI_STEERING_LIMITS_CAN_CANFD_BLENDED = HYUNDAI_LIMITS(384, 3, 5, 250);
