...
static uint8_t hyundai_get_counter(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x260U) {
    ...
    cnt = hyundai_can_canfd_blended ? ((msg->data[1] >> 4)  & 0xFU) : (msg->data[7] & 0xFU);
...
static bool hyundai_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == 0x420U && hyundai_can_canfd_blended) {
    int desired_accel_raw = (((msg->data[4] & 0x3FU) << 5) | (msg->data[3]) >> 3) - 1023U;
    int desired_accel_val = (((msg->data[3] & 0x7U) << 8) | msg->data[2]) - 1023U;
